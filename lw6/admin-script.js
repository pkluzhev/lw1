document.addEventListener("DOMContentLoaded", () => {
    ...
    function readImageFile(input, onImageProcessing, validSize, onInvalidSize) {
        ...
        if (imageSize < validSize) {
            ...
            reader.onload = function () {
                onImageProcessing(reader.result)
            }
    ...
    function onInvalidFileSize() {
        alertBanner.classList.add('alert-banner__show');
    }

    function uploadMainImage(event) {
        event.preventDefault()
        readImageFile(mainPostImage, onPostMainImageLoad, maxSizeFileLarge, onInvalidFileSize)
    }
    function uploadMainImageAgain(event) {
        event.preventDefault()
        readImageFile(uploadMainImgBut, onPostMainImageLoad, maxSizeFileLarge, onInvalidFileSize)
    }
    ...
    function uploadCardImage(event) {
        event.preventDefault()
        readImageFile(previewPostImage, onPostCardImageLoad, maxSizeFileSmall, onInvalidFileSize)
        
    }
    function uploadCardImageAgain(event) {
        event.preventDefault()
        readImageFile(uploadPreviewImgBut, onPostCardImageLoad, maxSizeFileSmall, onInvalidFileSize)
    }
    ...
    function uploadAuthorPhoto(event) {
        event.preventDefault()
        readImageFile(authorPhoto, onPostAuthorPhotoLoad, maxSizeFileSmall, onInvalidFileSize)
    }
    function uploadAuthorPhotoAgain(event) {
        event.preventDefault()
        readImageFile(uploadAuthorButton, onPostAuthorPhotoLoad, maxSizeFileSmall, onInvalidFileSize)
    }
